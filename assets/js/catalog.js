...
const join = (...parts) =>
  parts.filter(Boolean).map(String).join("/").replace(/\/+/g, "/");
        ...
        items.push({
          ...
          imageUrl:
            e.imageUrl && e.imageUrl.trim()
              ? e.imageUrl
              : `assets/img/database/${encodeURIComponent(displayName)}.png`,
...
function render() {
  ...
  const filtered = allItems.filter((it) => {
    ...
    const hay =
      `${it.name} ${it.rawKey || ""} ${it.category} ${it.component} ${it.description} ${(it.tags || []).join(" ")}`.toLowerCase();
